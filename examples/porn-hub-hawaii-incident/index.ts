...
// map chart ////////////////////////////////////////////////////////
let map = amcharts4.create("mapdiv", map.MapChart);
map.geoJSON = worldMap;
map.projection = new map.projections.Mercator();
map.deltaLongitude = 145;
map.seriesContainer.draggable = false;

let polygonSeries = map.series.push(new map.MapPolygonSeries());
polygonSeries.getDataFromJSON = true;
polygonSeries.exclude = ["ATA"];

let mapImageSeries = map.series.push(new map.MapImageSeries());
...
let mapLineSeries = map.series.push(new map.MapLineSeries());
...
function zoomToHawaii() {
	map.zoomToMapObject(honolulu, 4.5, true, 0);
}

// clock chart //////////////////////////////////////////////////////////////////
let clock = map.createChild(gauge.GaugeChart);
clock.width = 200;
clock.height = 200;
clock.align = "right";
clock.zIndex = 10;
...
function setTime() {
	...
	map.zoomLevel = 4.5 + slider.start;
	...
	let geoPoint = map.seriesPointToGeo(oPoint);
