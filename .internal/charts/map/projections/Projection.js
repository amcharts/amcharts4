...
var Projection = /** @class */ (function () {
    ...
    Projection.prototype.projectGeoLine = function (geoLine) {
        var convertedPoints = this.convertGeoLine(this.clipGeoLine(geoLine));
        return convertedPoints;
    };
    ...
    Projection.prototype.clipGeoLine = function (geoLine) {
        ...
        // temp, remove duplicates, @todo: review clipping algorithm to avoid this
        if (clippedLine.length > 1) {
            for (var i = 1; i < clippedLine.length; i++) {
                var prevSegment = clippedLine[i - 1];
                var segment = clippedLine[i];
                if (prevSegment.toString() == segment.toString()) {
                    clippedLine.splice(i, 1);
                    i--;
                }
            }
        }
    ...
    Projection.prototype.clipLine = function (subjectPolyline, clipPolygon) {
        ...
        for (var j = 0; j < clipPolygon.length; j++) {
            var clipPointA = clipPolygon[j - 1] || clipPolygon[clipPolygon.length - 1];
            var clipPointB = clipPolygon[j];
            var clipAngle = $math.round($math.getAngle($geo.geoToPoint(clipPointA), $geo.geoToPoint(clipPointB)), 5) % 360;
            for (var i = 1; i < segment.length; i++) {
                var pointA = segment[i - 1];
                var pointB = segment[i];
                if ($math.round($math.getAngle($geo.geoToPoint(pointA), $geo.geoToPoint(pointB)), 5) % 360 == clipAngle) {
                    segments.push(segment.splice(0, i));
                }
            }
            if (segment.length > 0) {
