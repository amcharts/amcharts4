...
interface IAdapterCallback<Target, T> {
	...
	key: any
	callback: any
	// callback<Key extends keyof T>(value: T[Key], target: Target, key?: Key): T[Key];
	scope: any;
	priority: number;
}
...
interface IGlobalAdapterCallback {
	id: number;
	key: string;
	callback<T, Target, Key extends keyof T, C>(this: C, value: T[Key], target: Target, key?: Key): T[Key];
	scope: any;
	priority: number;
	type: { new(): any };
}
	...
	public applyAll<T, Target, Key extends keyof T = keyof T>(type: Target, key: Key, value: T[Key]): T[Key] {
		...
		for (var i = 0; i < length; ++i) {
			...
			if (item.key === key && type instanceof item.type) {
				value = item.callback.call(item.scope, value, type, key) as T[Key];
			}
		}
