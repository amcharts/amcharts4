...
let bullet = chart.links.template.bullets.push(new sankey.LabelBullet());
bullet.label.text = "$ {value}";
bullet.label.fill = amcharts4.color("#ffffff");
...
chart.events.on("inited", () => {
	for (let i = 0; i < chart.links.length; i++) {
		...
		let bullet: sankey.LabelBullet = <sankey.LabelBullet>link.bullets.getIndex(1);
		bullet.opacity = 0;


		if (link.dataItem.toNode && link.dataItem.value > 20) {
...
function firstHalfAnimation(bullet) {
	...
	let animation = bullet.animate([{ property: "locationX", from: 0.2, to: 0.5 }, { property: "opacity", from: 0, to: 0.4 }], duration)
	animation.events.on("animationend", (event) => {
		secondHalfAnimation(event.target.object, duration);
	})
}

function secondHalfAnimation(bullet, duration: number) {
	let animation = bullet.animate([{ property: "locationX", from: 0.5, to: 0.8 }, { property: "opacity", from: 0.4, to: 0 }], duration)
	animation.events.on("animationend", (event) => {
		setTimeout(() => {
