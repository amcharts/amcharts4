...
let mapChart = amcharts4.create("chartdiv", maps.MapChart);
...
// countries
let countriesSeries = mapChart.series.push(new maps.MapPolygonSeries());
...
// night series
let nightSeries = mapChart.series.push(new maps.MapPolygonSeries());
...
// images series
let imagesSeries = mapChart.series.push(new maps.MapImageSeries())
...
function updateDateNight(time: number) {
    ...
    while (latitude < 180) {
        ...
        let point = maps.geo.normalizePoint({ longitude: longitude, latitude: latitude });
...
function solarPosition(time) {
    ...
    return maps.geo.normalizePoint({ longitude: longitude - equationOfTime(centuries) * amcharts4.math.DEGREES, latitude: solarDeclination(centuries) * amcharts4.math.DEGREES });
};
