...
let valueAxisX = chart.xAxes.push(new xy.ValueAxis());
let valueAxisY = chart.yAxes.push(new xy.ValueAxis());
...
circle.fill = amcharts4.color("#ff0000");
circle.propertyFields.fill = "color";
circle.stroke = amcharts4.color("#ffffff");
...
circle.adapter.add("radius", (radius, target) => {
	let dataItem = <xy.XYSeriesDataItem>(target.dataItem);
	if (dataItem) {
		return Math.max(2, 60 * dataItem.value / series.dataItem.values.value.high);
	}
})

circle.adapter.add("tooltipY", (tooltipY, target) => {
