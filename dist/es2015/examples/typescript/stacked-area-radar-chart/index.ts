...
bullet.adapter.add("dx", (dx, target) => {
  let angle = categoryAxis.getAngle(<am4charts.XYSeriesDataItem>target.dataItem, "categoryX", 0.5);
...
bullet.adapter.add("rotation", (dy, target) => {
  let angle = Math.min(chart.endAngle, Math.max(chart.startAngle, categoryAxis.getAngle(<am4charts.XYSeriesDataItem>target.dataItem, "categoryX", 0.5)));
  return angle;
})
